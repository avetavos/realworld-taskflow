...
- Identical versions (`postgres:18`, `redis:8`) for every developer — no "works on my Postgres 14."
...
- `redis:8` needs no environment variables or auth for local development — it matches `REDIS_URL=redis://localhost:6379`.
