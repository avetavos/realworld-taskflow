...
### Axum 0.8 and the 2024 edition

`edition = "2024"` is the current Rust edition (stable since Rust 1.85); nothing in TaskFlow's code hits its breaking changes (`unsafe extern` blocks, `gen` becoming a reserved keyword, return-position `impl Trait` capturing every in-scope lifetime), so it's a free upgrade. `axum = "0.8"` is the current major release. If you're coming from tutorials written for Axum 0.7, four things changed that this guide already accounts for:

- Path parameters use braces: `/boards/{id}` instead of `/boards/:id`, and wildcards are `/{*rest}`.
- `FromRequestParts` / `FromRequest` are native `async fn` traits — no `#[async_trait]` attribute.
- WebSocket `Message::Text` carries `Utf8Bytes` (not `String`) and `Message::Binary` carries `Bytes`.
- `Option<Path<T>>` no longer swallows extraction errors, and handlers must be `Sync`.
