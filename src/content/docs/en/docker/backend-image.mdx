...
**Multi-stage build — a `rust:1.98-bookworm` builder, a `debian:bookworm-slim` runtime (what we're using) vs. a single-stage image built on `rust:1.98-bookworm` throughout**
