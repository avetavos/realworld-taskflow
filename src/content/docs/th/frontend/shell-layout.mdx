...
`Base.astro` ใต้ `frontend/src/layouts/` — layout ที่ทุกหน้าในโมดูลนี้ห่อตัวเองด้วย ประกอบด้วย HTML shell (`<html>`, `<head>`, `<body>`), `<slot />` สำหรับเนื้อหาของแต่ละหน้า, nav bar ที่มีสองสถานะ — "Log in" / "Register" ตอนที่ยังไม่มี token เก็บอยู่ กับ "Boards" / "Log out" ตอนที่มี — และ `frontend/src/styles/global.css` สไตล์ชีตแบบ global เล็ก ๆ ที่ import เข้ามาครั้งเดียวในบทนี้

`frontend/astro.config.mjs` มี integration ของ `@astrojs/preact` ต่อไว้แล้วตั้งแต่ Module 1 แต่บทนี้ไม่ได้แตะ Preact เลย — `Base.astro` และทุกหน้าที่ layout นี้ห่อไว้ในโมดูลนี้ เป็น Astro ธรรมดาบวก `<script>` แบบ inline หนึ่งตัว บอร์ด Kanban แบบ reactive ที่ลาก-วางได้เป็นปัญหาคนละแบบจริง ๆ และต้องใช้เครื่องมือคนละแบบด้วย: Module 9, The Kanban Island
...
นี่ไม่ใช่ hydration mismatch แบบที่เจอใน React/Preact — ไม่มี framework มา re-render component tree ฝั่ง client ที่ต้องตรงกับสิ่งที่ server สร้างไว้ ตรงนี้เป็น HTML ธรรมดาที่มี attribute `hidden` กับ script ที่เอา attribute นั้นออก เลยไม่มีอะไรให้ "mismatch" กันตั้งแต่ต้น
...
`.nav-group[hidden] { display: none; }` คือกฎที่ควรหยุดดูสักหน่อย: ถ้าไม่มีบรรทัดนี้ `.nav-group { display: flex; }` ด้านบนจะชนะ cascade เหนือ `[hidden] { display: none; }` ที่เป็นค่าเริ่มต้นของ browser — ทั้งสองกฎมี specificity เท่ากัน (class selector หรือ attribute selector อย่างละหนึ่งตัว) และ author stylesheet ชนะ user-agent stylesheet เสมอเมื่อ specificity เท่ากัน compound selector อย่าง `.nav-group[hidden]` มี specificity สูงกว่า `.nav-group` เดี่ยว ๆ กฎนี้จึงชนะแทน และ `hidden` ก็ซ่อน element ได้จริง
...
`import '../styles/global.css';` ใน frontmatter ไม่ใช่ `<link rel="stylesheet">` ธรรมดาที่ชี้ไปยัง path ใต้ `/src/` — `src/styles/global.css` ไม่ได้เสิร์ฟตรง ๆ แบบไฟล์ใต้ `public/` แต่ต้องผ่าน asset pipeline ของ Vite การ `import` ไฟล์ `.css` ใน frontmatter ของ Astro ทำแบบนั้นเป๊ะ ๆ: Vite bundle ให้แล้วแทรก `<link>` tag ที่ได้ให้อัตโนมัติ ไม่ต้องคอย sync path เองด้วยมือ
