...
ไม่มีอะไรใหม่ — `frontend/src/lib/api.ts` มีอยู่แล้ว ถูกสร้างใน [auth-pages](/taskflow/th/frontend/auth-pages/) โดยความจำเป็น เพราะ `login.astro` กับ `register.astro` เรียก backend ไม่ได้เลยถ้าไม่มีไฟล์นี้ บทนี้คือ deep dive ที่บทนั้นเลื่อนมาทำ: ทุก export ในไฟล์นี้ ทำไม `ApiError` ถึงเป็น class แทนที่จะเป็น object ธรรมดา เกิดอะไรขึ้นกับ response body ตอนล้มเหลวเทียบกับตอนสำเร็จ และ `PUBLIC_API_URL` เดินทางจาก `.env` เข้าไปในการเรียก `fetch` จริง ๆ ได้ยังไง
...
ตอนที่ยังไม่ได้ login คำสั่งนี้จะ reject ด้วย `ApiError` ที่มี `.status` เป็น `401` ให้ login ก่อน (ผ่าน `/login`) แล้วรันใหม่ คราวนี้จะ resolve เป็น array อะไรก็ตามที่ `GET /boards` ([boards](/taskflow/th/rest-api/boards/)) คืนให้ user คนนั้น — `[]` สำหรับบัญชีที่เพิ่งสร้างใหม่
