...
`delete_column` invalidate เฉพาะในกิ่ง `if` เท่านั้น — กิ่งที่แถวถูกลบจริง ๆ กิ่ง `else` (ที่ `repo::delete_column` ไม่ได้ลบอะไรเลยเพราะ id หายไปแล้ว) ไม่มี tree change ให้ invalidate จึงคืน `AppError::NotFound` เหมือนเดิมจาก [columns](/taskflow/th/rest-api/columns/) โดยไม่เปลี่ยนแปลง
...
`move_card` รับ `state: &AppState` อยู่แล้ว — เป็นความต่างของ signature ที่ตั้งใจไว้ตัวเดียวที่ [move-reorder](/taskflow/th/rest-api/move-reorder/) เคยชี้ไว้ล่วงหน้า เพื่อให้การเพิ่มแบบนี้ในภายหลังไม่ต้องแตะ signature อีก เพิ่ม call invalidation ทันทีหลัง write ตรงจุดที่ comment ของบทนั้นทำเครื่องหมายไว้อยู่แล้ว:
