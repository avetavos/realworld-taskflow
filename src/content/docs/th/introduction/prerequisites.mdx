...
- [ ] **Node 24+** — รัน frontend ที่ทำด้วย Astro พร้อมเครื่องมือรอบ ๆ
...
Rust คือภาษาของ backend ใน TaskFlow ตัว `rustup` ทำหน้าที่ติดตั้งและจัดการ toolchain ส่วน `cargo` คือเครื่องมือ build และตัวจัดการแพ็กเกจ ติดตั้งจาก [rustup.rs](https://rustup.rs):
...
Node รัน dev server และเครื่องมือ build ของ frontend ที่ทำด้วย Astro โดย TaskFlow ต้องการ **Node 24 ขึ้นไป** ติดตั้งผ่าน Homebrew (หรือ [nodejs.org](https://nodejs.org) หรือ version manager อย่าง `nvm`):
...
Docker รัน PostgreSQL และ Redis ระหว่างการพัฒนา แล้วแพ็กทั้งสแตกไว้สำหรับโมดูล Docker ในภายหลัง ติดตั้ง **Docker Desktop for Mac** จาก [docker.com](https://www.docker.com/products/docker-desktop/) (หรือ `brew install --cask docker`) แล้วเปิดขึ้นมาหนึ่งครั้งเพื่อให้ engine ทำงาน
...
toolchain บน macOS ของคุณติดตั้งครบและ **ตรวจสอบ** เรียบร้อย: Rust (`rustc`/`cargo`) สำหรับ backend, Node 24+ สำหรับ frontend ที่ทำด้วย Astro, Docker Desktop (engine + Compose) เพื่อรันเซอร์วิสต่าง ๆ, `sqlx-cli` สำหรับ migration และ `psql`/`redis-cli` เพื่อตรวจสอบ data store

จบโมดูล 0 แล้ว ต่อไปคือ [โมดูล 1 · Setup & Tooling](/taskflow/th/setup/) ที่เราจะสร้างโครงของโปรเจกต์จริง
