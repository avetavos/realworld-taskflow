...
`create_pg_pool` เป็น `async` เพราะ `PgPoolOptions::connect` เปิด network connection จริงและ await handshake `create_redis_pool` ไม่ใช่ `async` — `deadpool_redis::Config::create_pool` แค่สร้างโครงสร้างของ pool และตรวจสอบ URL เท่านั้น ยังไม่เปิด connection จนกว่าจะมีการเรียก `pool.get().await` ครั้งแรก ซึ่งเราไม่ได้เรียกที่นี่
