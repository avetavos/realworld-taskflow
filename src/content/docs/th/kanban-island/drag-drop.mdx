...
`frontend/src/components/Board.tsx` — Preact island ที่ [board-page](/taskflow/th/kanban-island/board-page/) mount ด้วย `client:only="preact"` บทนี้พา component ตัวนั้นจากไฟล์เปล่าไปเป็นบอร์ดที่ทำงานได้เต็มรูปแบบ: fetch tree, render column พร้อม card ข้างใน และให้ user ลากการ์ดไปตำแหน่งใหม่ — ที่ไหนก็ได้ใน column เดิม หรือย้ายไป column อื่น — โดยการย้ายขึ้นหน้าจอทันที และ persist ด้วยการเรียก `PATCH /cards/:id/move` หนึ่งครั้ง ([move-reorder](/taskflow/th/rest-api/move-reorder/))

สิ่งที่บอร์ดนี้ยังทำไม่ได้คือรับรู้การย้ายของ *คนอื่น* สองแท็บที่เปิดบอร์ดเดียวกัน หรือสองคนที่แก้ไขพร้อมกัน จะไม่เห็นการเปลี่ยนแปลงของกันและกันจนกว่าจะ reload ด้วยมือ — [live-sync](/taskflow/th/kanban-island/live-sync/) บทถัดไป ต่อฝั่ง WebSocket ที่ปิดช่องว่างนั้น
...
เพิ่ม interface สามตัวนี้ใน `frontend/src/lib/api.ts` ต่อจาก interface `Board` ที่ [api-client](/taskflow/th/frontend/api-client/) สร้างไว้ — ทั้งสามสะท้อน `Card`, `Column`, และ `BoardTree` ของ `boards::model` ([boards](/taskflow/th/rest-api/boards/)) ทีละ field:
...
ความสะดวกนี้มีต้นทุนจริงที่ [live-sync](/taskflow/th/kanban-island/live-sync/) จะระบุชัด: event realtime `column.created`/`column.updated` broadcast `Column` แบบ *เปล่า* ที่ไม่มี field `cards` เลย ทั้งที่ type TypeScript ตัวนี้อ้างว่ามีเสมอ
...
5. **`pendingRef.current.add(cardId)`** รันทันทีก่อน `setBoard` แบบ optimistic — นี่คือ Set ที่ [live-sync](/taskflow/th/kanban-island/live-sync/) จะเอาไปใช้ซ้ำเพื่อจำ echo ของ WebSocket ตัวเอง; สำหรับบทนี้เพียงบทเดียว Set นี้มีไว้ให้ `try`/`finally` ด้านล่างบันทึกว่า "มีการย้ายของการ์ดใบนี้ค้างอยู่" เท่านั้น
