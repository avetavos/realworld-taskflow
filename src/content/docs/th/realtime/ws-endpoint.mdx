...
description: สร้าง realtime/handlers.rs — การ upgrade ของ GET /ws/boards/{id}?token=, การยืนยันตัวตนผ่าน query parameter, การตรวจสอบสมาชิกภาพ และ tokio::select! loop ต่อ socket — พร้อมทั้ง Hub registry และฟิลด์บน AppState ที่ต้องพึ่งพา
sidebar:
  order: 2
---
...
ไล่ดู `ws_upgrade` ตามลำดับ — รูปร่างเหมือน `AuthUser::from_request_parts` จาก [middleware](/taskflow/th/auth/middleware/) บวกอีกหนึ่งขั้นตอน:

1. **ตรวจสอบ JWT** จาก query parameter `token` เหมือนกับที่ `AuthUser` ตรวจสอบ bearer token ใน header `Authorization`
2. **ตรวจสอบ Redis allowlist** (`auth:token:{jti}`) — token ที่ถูกเพิกถอนแล้ว (logout หรือการกระทำของ admin ในอนาคต) ก็โดนปฏิเสธที่นี่เช่นกัน ไม่ใช่แค่ตอน REST call
...
- **`rx.recv()`** — event ถัดไปที่ `Hub::subscribe` จะส่งให้บอร์ดนี้ เมื่อรับสำเร็จก็ส่งต่อไปยังไคลเอนต์เป็น `Message::Text` `RecvError::Lagged` (socket นี้ตามหลังเกิน `CHANNEL_CAPACITY` ข้อความ) *ไม่ใช่* ข้อผิดพลาดร้ายแรง — `continue` จะดึงข้อความถัดไปที่มีต่อ; socket ที่ตามหลัง event ที่มาถี่ยิบ 128 ครั้งบนบอร์ดที่คึกคักมากจะพลาด state ระหว่างทางไปบ้าง แต่ยังทำงานต่อได้ และ [reconnect](/taskflow/th/realtime/reconnect/) จะอธิบายว่าทำไมนี่ถึงเป็นช่องว่างที่ยอมรับได้และเยียวยาตัวเองได้ ไม่ใช่บั๊กที่ต้องกำจัด `RecvError::Closed` (ทุก `Sender` ของ channel นี้ถูก drop ไปแล้ว) จะจบ loop — สิ่งนี้เกิดขึ้นไม่ได้ในการออกแบบปัจจุบันของโมดูลนี้ เพราะ `Hub` เองถือ `Sender` ไว้ตลอด แต่ match ก็ต้องครอบคลุมทุกกรณีอยู่ดี
...
- **`json.into()`** — `Message::Text` ของ Axum 0.8 ถือ `Utf8Bytes` (byte buffer ที่ตรวจแล้วว่าเป็น UTF-8 และ clone ถูก) แทน `String` ดังนั้น `String` ที่ได้จาก broadcast channel จึงถูกแปลงด้วย `.into()` ก่อนส่งออก ส่วน `Message::Binary` ก็ถือ `Bytes` ในทำนองเดียวกัน
