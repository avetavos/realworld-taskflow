...
`BoardEvent` ในฐานะ struct ของ Rust จะกลายเป็นโค้ดจริงใน [ws-endpoint](/taskflow/th/realtime/ws-endpoint/) — บทนี้กำหนดรูปร่างไว้ครั้งเดียว เพื่อให้สามบทถัดไปสร้างต่อยอดได้โดยไม่ต้องนิยามซ้ำ
...
- ฟิลด์ไหนใน `BoardEvent` ที่บอกไคลเอนต์ว่า event นี้เป็นของบอร์ดไหน และฟิลด์นั้นตรงกับชื่อ Redis pub/sub **channel** หรือชื่อ Redis cache **key** จาก [cache-reads](/taskflow/th/caching/cache-reads/)? (`board_id` ตรงกับชื่อ channel `board:{board_id}` — *ไม่ใช่* cache key `cache:board:{board_id}` ที่เป็นคนละ namespace ของ Redis โดยสิ้นเชิง)
