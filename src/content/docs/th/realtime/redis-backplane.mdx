...
description: ขยาย hub.rs ด้วย subscriber task ที่ PSUBSCRIBE board:* และเพิ่ม realtime::publish เชื่อมเข้ากับ move_card และ mutation อื่น ๆ ทุกตัวในโมดูล 5 เพื่อให้ event ไปถึง socket บน backend instance ทุกตัว ไม่ใช่แค่ตัวที่จัดการ write
sidebar:
  order: 3
---
...
การเรียก `realtime::publish` ใน `move_card` แทนที่คอมเมนต์ `// Module 7 wires realtime broadcast of card.moved here` ที่ [move-reorder](/taskflow/th/rest-api/move-reorder/) ทิ้งไว้ตรงจุดเดิมเป๊ะ — signature `&AppState` ที่บทนั้นวางไว้ล่วงหน้าสองโมดูล ในที่สุดก็ได้ใช้ตามเหตุผลที่เพิ่มเข้ามาตั้งแต่แรก
