...
### Axum 0.8 กับ edition 2024

`edition = "2024"` คือ Rust edition ปัจจุบัน (stable ตั้งแต่ Rust 1.85) โค้ดของ TaskFlow ไม่โดน breaking change ของมันเลย (`unsafe extern` block, `gen` กลายเป็น keyword สงวน, `impl Trait` ในตำแหน่ง return จับทุก lifetime ใน scope) จึงอัปเกรดได้ฟรี ๆ ส่วน `axum = "0.8"` คือ major release ปัจจุบัน ถ้าคุณมาจาก tutorial ที่เขียนสำหรับ Axum 0.7 มีสี่อย่างที่เปลี่ยนไปและคู่มือนี้ปรับให้แล้ว:

- path parameter ใช้วงเล็บปีกกา: `/boards/{id}` แทน `/boards/:id` และ wildcard เป็น `/{*rest}`
- `FromRequestParts` / `FromRequest` เป็น trait ที่มี `async fn` แบบ native — ไม่ต้องมี attribute `#[async_trait]`
- `Message::Text` ของ WebSocket ถือ `Utf8Bytes` (ไม่ใช่ `String`) และ `Message::Binary` ถือ `Bytes`
- `Option<Path<T>>` ไม่กลืน error ของการ extract อีกต่อไป และ handler ต้องเป็น `Sync`
